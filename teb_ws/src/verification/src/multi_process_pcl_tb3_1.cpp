...
std::string frame_id="tb3_1_tf/camera_rgb_optical_frame";
pcl::PointCloud<pcl::PointXYZ> pass_filtered_cloud;
static const std::string PCL_TOPIC = "/tb3_1/camera/depth/points";
...
int main(int argc, char **argv)
{
   ...
   ros::init (argc, argv, "tb3_1_cloud_processing");
   ros::NodeHandle nh;

   //subscribe
   ros::Subscriber yolo_sub = nh.subscribe("tb3_1/H_Detection_msg", 10, human_callback);
   ros::Subscriber PCLsub = nh.subscribe(PCL_TOPIC, 10, callback);
   ros::Subscriber cp_flag_sub = nh.subscribe("tb3_1/cp_flag", 10, cp_flag_callback);
   ...
   //publish
   pub_cropped_cloud=nh.advertise<sensor_msgs::PointCloud2>("tb3_1/cropped_cloud",1);
   pub_extracted_cloud=nh.advertise<sensor_msgs::PointCloud2>("tb3_1/extracted_cloud",1);
   pub_projected_cloud=nh.advertise<sensor_msgs::PointCloud2>("tb3_1/projected",1);
   passthrough_filtered=nh.advertise<sensor_msgs::PointCloud2>("tb3_1/passfiltered",1);
   passthrough_filtered_again=nh.advertise<sensor_msgs::PointCloud2>("tb3_1/passfiltered_again",1);
